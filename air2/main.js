...
const NET_SEND_INTERVAL_MS = 45;
const NET_CODE_PREFIX = 'air-';
...
const net = {
    ...
    conn: null,
    roomCode: '',
    remotePaddle: { detected: false, isPinching: false, world: new THREE.Vector3(4, 0, 0) },
    lastSend: 0,
    lastStateFromHost: null,
    lastBroadcast: 0
};
...
const setNetStatus = text => {
    if (netEls.netStatus) netEls.netStatus.textContent = text;
};

const setCodeDisplay = code => {
    const pretty = code ? code.split('').join(' ') : '— — — —';
    if (netEls.codeDisplay) netEls.codeDisplay.textContent = `Code: ${pretty}`;
};

const randomCode = () => Math.floor(1000 + Math.random() * 9000).toString();

const sanitizeCode = raw => (raw || '').replace(/\D/g, '').slice(0, 4);
...
const updatePaddleFromHand = (side, hand) => {
    ...
    return { x: targetX, y: targetY, detected: hand.detected };
};
...
const applyHostState = state => {
    if (!state || !ball.body || !paddles.left.body || !paddles.right.body) return;

    const puck = state.puck || {};
    ball.body.setTranslation({ x: puck.x || 0, y: puck.y || 0, z: puck.z || 0 }, true);
    ball.body.setLinvel({ x: puck.vx || 0, y: puck.vy || 0, z: puck.vz || 0 }, true);
    if (ball.mesh) {
        ball.mesh.position.set(puck.x || 0, puck.y || 0, puck.z || 0);
    }

    const lp = state.left || { x: -4, y: 0 };
    paddles.left.body.setNextKinematicTranslation({ x: lp.x, y: lp.y, z: 0 });
    paddles.left.mesh.position.set(lp.x, lp.y, 0);
    paddles.left.lastPos.set(lp.x, lp.y, 0);

    const rp = state.right || { x: 4, y: 0 };
    paddles.right.body.setNextKinematicTranslation({ x: rp.x, y: rp.y, z: 0 });
    paddles.right.mesh.position.set(rp.x, rp.y, 0);
    paddles.right.lastPos.set(rp.x, rp.y, 0);
    ...
    if (state.statusText) setStatus(state.statusText);
};
...
const clearNet = () => {
    ...
    net.conn = null;
    net.peer = null;
    net.remotePaddle = { detected: false, isPinching: false, world: new THREE.Vector3(4, 0, 0) };
    net.lastStateFromHost = null;
    net.roomCode = '';
    net.role = 'solo';
    setNetStatus('Play solo or share a code.');
    setCodeDisplay('');
};

const handleNetMessage = msg => {
        ...
        case 'paddle': {
            if (net.role === 'host') {
                net.remotePaddle = {
                    ...
                    isPinching: msg.isPinching,
                    world: new THREE.Vector3(msg.x || 4, msg.y || 0, 0)
                };
            }
            break;
        }
        case 'state': {
            net.lastStateFromHost = msg;
            if (net.role === 'client') {
                applyHostState(msg);
            }
            break;
...
const setupConnection = conn => {
    ...
    conn.on('open', () => {
        setNetStatus(net.role === 'host' ? 'Player connected.' : 'Connected to host. You are Right.');
        setStatus(net.role === 'host' ? 'Hosting: you are Left paddle.' : 'Joined: you are Right paddle.');
    });

    conn.on('data', handleNetMessage);

    conn.on('close', () => {
        setNetStatus('Disconnected. Back to solo.');
        setStatus('Disconnected. Solo mode.');
        clearNet();
    });

    conn.on('error', err => {
        console.error('Connection error', err);
        setNetStatus('Connection error. Try again.');
    });
};

const broadcastStateFromHost = () => {
    if (!net.conn || !net.conn.open || !ball.body) return;
    const pos = ball.body.translation();
    const vel = ball.body.linvel();
    netSend({
        type: 'state',
        puck: { x: pos.x, y: pos.y, z: pos.z, vx: vel.x, vy: vel.y, vz: vel.z, active: ballActivated },
        left: { x: paddles.left.lastPos.x, y: paddles.left.lastPos.y },
        right: { x: paddles.right.lastPos.x, y: paddles.right.lastPos.y },
        scores: { left: scores.left, right: scores.right },
        statusText: statusEl.textContent
    });
};

const tryHost = (attempt = 0) => {
    if (!window.Peer) {
        setNetStatus('PeerJS failed to load.');
        return;
    }
    const code = randomCode();
    setCodeDisplay(code);
    setNetStatus('Starting host...');
    net.roomCode = code;
    net.role = 'host';
    if (net.peer) {
        net.peer.destroy();
    }
    net.peer = new Peer(`${NET_CODE_PREFIX}${code}`, { debug: 1 });

    net.peer.on('open', id => {
        setNetStatus('Hosting. Share the code.');
    });

    net.peer.on('connection', setupConnection);
    ...
    net.peer.on('error', err => {
        ...
        setNetStatus('Host failed. Try again.');
    });
};

const tryJoin = rawCode => {
    const code = sanitizeCode(rawCode);
    if (!code || code.length < 4) {
        setNetStatus('Enter a 4-digit code.');
        return;
    }
    if (!window.Peer) {
        setNetStatus('PeerJS failed to load.');
    ...
    net.peer.on('open', () => {
        const conn = net.peer.connect(`${NET_CODE_PREFIX}${code}`);
        setupConnection(conn);
    });
    net.peer.on('error', err => {
        console.error('Peer join error', err);
        setNetStatus('Join failed. Check code.');
    });
};

const setupNetUI = () => {
    if (!netEls.hostBtn) return;
    setCodeDisplay('');
    setNetStatus('Play solo or share a code.');

    netEls.codeInput.addEventListener('input', e => {
        e.target.value = sanitizeCode(e.target.value);
    });

    netEls.hostBtn.addEventListener('click', () => {
        clearNet();
        tryHost();
    });

    netEls.joinBtn.addEventListener('click', () => {
        const code = sanitizeCode(netEls.codeInput.value || randomCode());
        netEls.codeInput.value = code;
        tryJoin(code);
    });

    netEls.connectBtn.addEventListener('click', () => {
        tryJoin(netEls.codeInput.value);
    });
};
...
const animate = now => {
    if (net.role === 'host') {
        ...
        const remote = net.conn ? net.remotePaddle : handState.right;
        updatePaddleFromHand('right', remote);
        ...
        if (net.conn && net.conn.open && now - net.lastBroadcast > NET_SEND_INTERVAL_MS) {
            net.lastBroadcast = now;
            broadcastStateFromHost();
        }
    } else if (net.role === 'client') {
        const paddle = updatePaddleFromHand('right', handState.right) || { x: 4, y: 0, detected: false };
        if (net.conn && net.conn.open && now - net.lastSend > NET_SEND_INTERVAL_MS) {
            net.lastSend = now;
            netSend({ type: 'paddle', x: paddle.x, y: paddle.y, detected: paddle.detected });
        }

        if (net.lastStateFromHost) {
            applyHostState(net.lastStateFromHost);
        }
    } else {
        updatePaddleFromHand('left', handState.left);
